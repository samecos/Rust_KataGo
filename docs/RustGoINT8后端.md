...
最新交付见 [INT8 专项优化](RustGoINT8专项优化.md)：相对上一版同配置 INT8，全 FFN 的 B11 C1/C32 为 +2.99%/+1.58%，剪枝 B15 为 +4.06%/+2.15%；融合开关固定 batch 原始输出逐位相同。宽层混合配置没有新增收益，保留原路径。新构建位于 `target/int8-specialized/build/release/katago-rs.exe`，精度恢复和棋力收益仍未获证明。

初次交付的记录如下：B11 的 FFN INT8 在 C32 提高约 12.5%，剪枝 B15 则慢约 3.5%。用户随后授权了剪枝专项优化：该轮候选 B15 C32 为 **+3.26%**（全 FFN）或 **+5.43%**（宽层 INT8），B11 C32 为 **+17.78%**，均相对同配置默认 FP16。该轮二进制、配置、复测与 `0831.sgf` 校准未通过的边界见 [剪枝 B15 诊断](RustGoB15剪枝INT8诊断.md)。本文下方表格和旧二进制 SHA 保留初次交付历史，不是新候选的复测结果。两轮均未证明棋力净收益。
