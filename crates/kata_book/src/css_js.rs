//! CSS and JavaScript embedded in book HTML exports.
//!
//! Corresponds to `KataGo/cpp/book/bookcssjs.cpp`.
...
/// Returns the CSS used for book HTML rendering.
pub fn book_css() -> &'static str {
    BOOK_CSS
}

static BOOK_JS: LazyLock<String> = LazyLock::new(|| [BOOK_JS1, BOOK_JS2, BOOK_JS3].concat());

/// Returns the JavaScript used for book HTML rendering.
pub fn book_js() -> &'static str {
    BOOK_JS.as_str()
}
...
mod tests {
    ...
    #[test]
    fn css_js_are_non_empty() {
        assert!(!book_css().is_empty());
        assert!(!book_js().is_empty());
    }

    #[test]
    fn css_js_concat_matches_constants() {
        assert_eq!(book_js(), [BOOK_JS1, BOOK_JS2, BOOK_JS3].concat());
    }
}
