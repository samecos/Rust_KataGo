//! Independent diagnostic replay of the production TN, FP32-output Lt query.
...
mod precision;
use precision::{
    flags_support_fp16_fp32, ACCUMULATOR_32F, ACCUMULATOR_TYPE_MASK, INPUT_16F, INPUT_TYPE_MASK,
};
...
impl Descriptors {
    ...
    fn precision_readback(&self) -> Result<(bool, Value), String> {
        ...
        for (name, layout, expected) in [
            ...
            ("C", self.cd, sys::cudaDataType_t::CUDA_R_32F),
            ("D", self.cd, sys::cudaDataType_t::CUDA_R_32F),
        ] {
...
fn descriptor_json(m: usize, n: usize, k: usize) -> Value {
    json!({"compute": "CUBLAS_COMPUTE_32F", "scale": "CUDA_R_32F",
        ...
        "C_D": {"same_pointer": true, "dtype": "f32", "rows": n, "cols": m, "ld": n},
        "alpha": 1.0, "beta": 0.0})
}

/// Query REQ8 independently at the source shape. No algorithm is executed and
/// no state/flags predicate is added to the production workspace-only filter.
pub fn select(
    rt: &CudaRuntime,
    stream: &Arc<CudaStream>,
    m: usize,
    ...
    if count < 1 || count as usize > REQUESTED {
        return Err(format!("REQ8 returned invalid/empty count: {count}"));
    }
    heuristics.truncate(count as usize);
    // Intentionally do NOT filter state, waves, IDs, or capabilities. This is
    // the production selector's sole predicate and preserves original order.
    let eligible: Vec<usize> = heuristics
        .iter()
        .enumerate()
        .filter_map(|(i, h)| (h.workspaceSize <= WORKSPACE_BYTES).then_some(i))
        .collect();
    let selected = *eligible.first().ok_or("no workspace-eligible heuristic")?;
    ...
    let unfiltered: Vec<Value> = heuristics
        .iter()
        .enumerate()
        .map(|(i, h)| {
            json!({"original_index": i, "result": heuristic_json(h), "opaque": opaque(&h.algo),
            "workspace_eligible": h.workspaceSize <= WORKSPACE_BYTES})
        })
        .collect();
    let metadata = json!({
        "status": "SELECTED_NOT_EXECUTED", "executed": false, "requested_count": REQUESTED,
        "selection": {"filtered_index": 0, "original_index": selected,
            "filter": {"max_workspace_bytes": WORKSPACE_BYTES, "predicate": "workspaceSize <= max_workspace_bytes",
                "preserves_order": true, "filters_state": false}, "opaque_unchanged": true},
        "scope": "independent production-policy source-shape query; not the private production cache entry",
        "shape": shape_json(m, n, k), "selection_shape": shape_json(m, n, k), "execution_shape": null,
        "descriptor": descriptor_json(m, n, k),
        "cublaslt_version": unsafe { sys::cublasLtGetVersion() },
        "stream": format!("0x{:016x}", stream.cu_stream() as usize),
        "heuristic": {"api": "cublasLtMatmulAlgoGetHeuristic", "status": status_json(query_status),
            "requested": REQUESTED, "returned": count, "unfiltered": unfiltered,
            "filter": "workspaceSize <= 33554432 only, preserving original order",
            "eligible_original_indices": eligible, "selected_original_index": selected, "selected_filtered_index": 0},
        "selected": {"opaque": opaque(&algo), "opaque_before": opaque(&heuristics[selected].algo),
    ...
    Ok(Selection {
        ...
        numerical_flags,
        min_align: [min_a, min_b, min_c, min_d],
        metadata,
    })
}
...
/// Reuse the exact source object at the target M. An unsupported Check, dtype,
...
pub fn execute_selected(
    ...
    if [m, n, k].iter().any(|&v| v == 0 || v > i64::MAX as usize)
        || n != selection.source_shape[1]
        || k != selection.source_shape[2]
    {
        return Ok(not_supported(
            report,
            "only positive target M may vary; N/K and FP16/FP32 TN descriptors are fixed",
        ));
    }
    ...
    let output_stride = n.checked_mul(4).ok_or("float stride overflow")?;
    ...
    report["precision_guard"] = json!({"flags": selection.numerical_flags,
        ...
        "descriptor_scale": "CUDA_R_32F", "descriptor_output": "CUDA_R_32F"});
    ...
    if !descriptor_precision_ok {
        rejected.push("actual target descriptors do not specify FP16 A/B, FP32 C/D, COMPUTE_32F and FP32 scale".into());
    }
...
/// Original same-shape entry point: any unsupported/failed control remains an
/// error. Successful reports retain the r2 fields plus explicit source/target.
#[allow(clippy::too_many_arguments)]
pub fn replay(
    rt: &CudaRuntime,
    stream: &Arc<CudaStream>,
    ...
    let selection = select(rt, stream, m, n, k)?;
    let report = execute_selected(rt, stream, inputs, weights, out, m, n, k, &selection)?;
    if report["status"] != "PASS_REPLAY_EXECUTED" {
        return Err(format!(
            "same-shape replay did not execute successfully: {report}"
        ));
    }
    Ok(report)
}
