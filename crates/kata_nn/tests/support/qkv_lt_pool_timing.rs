...
// APPENDED_TIMING_PREPARATION: the complete frozen numeric helper above is unchanged.
// No numerical helper function is used in a measured interval.

pub struct PreparedLt {
    descriptors: Descriptors,
    stream: Arc<CudaStream>,
    handle: sys::cublasLtHandle_t,
    algo: sys::cublasLtMatmulAlgo_t,
    original: [u64; 8],
    pointers: [u64; 4], // input, weights, output, stream-private workspace
    alpha: f32,
    beta: f32,
    pub metadata: Value,
}

impl Drop for PreparedLt {
    fn drop(&mut self) {
        // Descriptors/original object remain live until queued work completes,
        // including unwinding. Normal callers already synchronized every stop.
        let _ = self.stream.synchronize();
    }
}

impl PreparedLt {
    pub fn pointers(&self) -> [u64; 4] { self.pointers }
    pub fn identity_unchanged(&self) -> bool { self.algo.data == self.original }

    /// Enqueue only. Caller must keep runtime, matrices, stream workspace and
    /// their DevicePtr/DevicePtrMut guards alive through an external stop-event
    /// synchronization (and synchronize on every failure path before release).
    /// Call only with the exact pointers checked at preparation, on this stream.
    pub unsafe fn submit(&self) -> Result<(), String> {
        status(unsafe {
            sys::cublasLtMatmul(
                self.handle, self.descriptors.operation, (&self.alpha as *const f32).cast(),
                self.pointers[1] as *const c_void, self.descriptors.a,
                self.pointers[0] as *const c_void, self.descriptors.b,
                (&self.beta as *const f32).cast(),
                self.pointers[2] as *const c_void, self.descriptors.cd,
                self.pointers[2] as *mut c_void, self.descriptors.cd,
                &self.algo, self.pointers[3] as *mut c_void, WORKSPACE_BYTES,
                self.stream.cu_stream() as sys::cudaStream_t,
            )
        }, "prepared original-object Lt enqueue")
    }
}

/// One descriptor creation and Check per candidate, outside all timing.
/// This does not execute a Matmul. The caller performs independent full-output
/// self-controls using submit before admitting the measured phases.
pub fn prepare_selected(
    ...
    selection: &Selection,
) -> Result<PreparedLt, String> {
    let [m,n,k] = selection.source_shape;
    if [m,n,k] != [5054,1152,384] || inputs.len() != m*k
        || weights.len() != n*k || output.len() != m*n+256 {
        return Err("prepared QKV shape/allocation mismatch".into());
    }
    if selection.source_state != 0 || selection.source_workspace_bytes > WORKSPACE_BYTES
        || !(0..=1).contains(&selection.splitk_num) || selection.reduction_scheme != 0
        || !flags_support_fp16_fp32(selection.numerical_flags) {
        return Err("prepared QKV original state/workspace/FP32/splitK/reduction admission rejected".into());
    }
    if !Arc::ptr_eq(stream.context(), &rt.device)
        || !Arc::ptr_eq(selection.stream.context(), &rt.device)
        || stream.cu_stream() != selection.stream.cu_stream()
        ...
        return Err("prepared QKV runtime/stream/buffer context mismatch".into());
    }
    stream.context().bind_to_thread().map_err(|e| e.to_string())?;
    let handle = rt.cublaslt_handle().ok_or("Lt handle missing")?;
    if handle as usize != selection.handle || rt.cublaslt_workspace_len() != WORKSPACE_BYTES {
        return Err("prepared QKV handle/workspace mismatch".into());
    }
    let descriptors = Descriptors::new(m,n,k)?;
    let (precision_ok, descriptor_readback) = descriptors.precision_readback()?;
    if !precision_ok { return Err("prepared QKV actual descriptor precision differs".into()); }
    let algo = selection.algo;
    if algo.data != selection.original { return Err("original algorithm changed before preparation".into()); }
    let (ip, _ig) = inputs.device_ptr(stream);
    let (wp, _wg) = weights.device_ptr(stream);
    let (op, _og) = output.device_ptr_mut(stream);
    let (workspace, _) = rt.cublaslt_workspace_ptr(stream);
    let mut alignment = Vec::new();
    for (name,pointer,stride,minimum) in [
        ("A_weights",wp,k*2,selection.min_align[0]),
        ("B_inputs",ip,k*2,selection.min_align[1]),
        ("C_output",op,n*2,selection.min_align[2]),
        ("D_output",op,n*2,selection.min_align[3]),
    ] {
        if minimum == 0 || pointer == 0 || pointer % minimum as u64 != 0 || stride % minimum as usize != 0 {
            return Err(format!("prepared {name} actual alignment rejected"));
        }
        alignment.push(json!({"matrix":name,"pointer":format!("0x{pointer:016x}"),
            "leading_dimension_bytes":stride,"minimum_alignment_bytes":minimum,
            "pointer_aligned":true,"stride_aligned":true}));
    }
    if workspace == 0 || workspace % 256 != 0 { return Err("prepared workspace alignment rejected".into()); }
    let mut checked: sys::cublasLtMatmulHeuristicResult_t = unsafe { std::mem::zeroed() };
    let check = unsafe { check_function()?(handle, descriptors.operation, descriptors.a, descriptors.b,
        descriptors.cd, descriptors.cd, &algo, &mut checked, stream.cu_stream() as sys::cudaStream_t) };
    status(check,"prepared CheckForStream")?;
    status(checked.state,"prepared CheckForStream state")?;
    if checked.workspaceSize > WORKSPACE_BYTES || !checked.wavesCount.is_finite()
        || algo.data != selection.original {
        return Err("prepared Check workspace/waves/original opaque rejected".into());
    }
    let mut metadata = selection.metadata.clone();
    metadata["status"] = json!("PREPARED_NOT_EXECUTED");
    metadata["descriptor_readback"] = descriptor_readback;
    metadata["alignment"] = json!(alignment);
    metadata["workspace"] = json!({"pointer":format!("0x{workspace:016x}"),"bytes":WORKSPACE_BYTES,
        "aligned_256":true,"same_runtime_and_stream":true});
    metadata["check_for_stream"] = json!({"called":true,"status":check as u32,
        "state":checked.state as u32,"result":heuristic_json(&checked),"result_algo_ignored":true});
    metadata["selected"]["opaque_after_prepare"] = opaque(&algo);
    metadata["preparation"] = json!({"descriptor_creations":1,"check_calls":1,"matmul_calls":0,
        "submit":"original-object cublasLtMatmul enqueue only; no descriptors, Check, allocation or synchronization",
        "external_access_guards_required_through_stop_sync":true});
    Ok(PreparedLt{descriptors,stream:Arc::clone(stream),handle,algo,original:selection.original,
        pointers:[ip,wp,op,workspace],alpha:1.0,beta:0.0,metadata})
}
