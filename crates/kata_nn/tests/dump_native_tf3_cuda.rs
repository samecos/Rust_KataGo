...
//!
//! KATAGO_RUN_NATIVE_TF3_DUMP=1 enables this test. Then model/GPU/configuration
//! errors fail loudly. KATAGO_TEST_MODEL_DIR must contain the SHA-bound model.
//! KATAGO_DUMP_BATCH=1|14|16 (default 1), KATAGO_DUMP_LANES=1|2 (default 1),
...
fn selected_tactics() -> BTreeMap<String, Value> {
    ...
    for (key, default, allowed) in [
        ...
        ("KATAGO_CUDA_GATE_ROWQUAD_R1", "0", &["0", "1"][..]),
        ("KATAGO_CUDA_QKV_IMMUTABLE_R1", "0", &["0", "1"][..]),
        ("KATAGO_CUDA_QKV_CLASSIC_N128_R1", "0", &["0", "1"][..]),
        ("KATAGO_CUDA_OUTPROJ_CLASSIC_N128_R1", "0", &["0", "1"][..]),
...
#[test]
fn dump_native_tf3_cuda() {
    match std::env::var("KATAGO_RUN_NATIVE_TF3_DUMP") {
        Err(std::env::VarError::NotPresent) => {
            eprintln!("skipped: set KATAGO_RUN_NATIVE_TF3_DUMP=1 in an exclusive GPU window");
            return;
        }
        Ok(value) if value == "1" => {}
        value => panic!("KATAGO_RUN_NATIVE_TF3_DUMP must be 1 when set: {value:?}"),
    }
    let batch = integer_env("KATAGO_DUMP_BATCH", 1, &[1, 14, 16]);
    let lane_count = integer_env("KATAGO_DUMP_LANES", 1, &[1, 2]);
    ...
    let mut meta = json!({
        ...
        "dump_test_source_sha256":hex::encode(Sha256::digest(include_bytes!("dump_native_tf3_cuda.rs"))),
        ...
        "execution_marker_verification":"external [cuda-tactic] log verification required; no canary or workspace modification",
        "lane_resources":[],"rows":[]
    });
